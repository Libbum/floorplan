...
import TypedSvg.Attributes exposing (class, cx, cy, d, opacity, points, r, viewBox, width, x, y)
...
svgMedia : ( Float, Float ) -> Float -> String -> List (Svg msg) -> Html msg
svgMedia ( xPos, yPos ) xBox className =
    svg
        [ x (px xPos)
        , y (px yPos)
        , class [ "media", className ]
        , viewBox 0 0 xBox 512
        , width (px 25)
        ]
...
projector : ( Float, Float ) -> Html msg
projector coords =
    svgMedia coords
        648
        "projector"
